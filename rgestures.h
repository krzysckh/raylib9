...
typedef struct {
  ...
  GETS(Vector2) position[MAX_TOUCH_POINTS];
} GestureEvent;
...
typedef struct {
    ...
    struct {
        ...
        GETS(Vector2) upPosition;             // Touch up position
        GETS(Vector2) downPositionA;          // First touch down position
        GETS(Vector2) downPositionB;          // Second touch down position
        GETS(Vector2) downDragPosition;       // Touch drag position
        GETS(Vector2) moveDownPositionA;      // First touch down position on move
        GETS(Vector2) moveDownPositionB;      // Second touch down position on move
        GETS(Vector2) previousPositionA;      // Previous position A to compare for pinch gestures
        GETS(Vector2) previousPositionB;      // Previous position B to compare for pinch gestures
    ...
    struct {
        GETS(Vector2) vector;                 // DRAG vector (between initial and current position)
    ...
    struct {
        GETS(Vector2) vector;                 // PINCH vector (between first and second touch points)
...
//----------------------------------------------------------------------------------
static float rgVector2Angle(GETS(Vector2) initialPosition, GETS(Vector2) finalPosition);
static float rgVector2Distance(GETS(Vector2) v1, GETS(Vector2) v2);
...
void ProcessGestureEvent(GestureEvent event)
{
    ...
    if (GESTURES.Touch.pointCount == 1)     // One touch point
    {
        if (event.touchAction == TOUCH_ACTION_DOWN)
        {
            ...
            GESTURES.Drag.vector = (GETS(Vector2)){ 0.0f, 0.0f };
        }
        else if (event.touchAction == TOUCH_ACTION_UP)
        {
            ...
            GESTURES.Touch.downDragPosition = (GETS(Vector2)){ 0.0f, 0.0f };
    ...
    else if (GESTURES.Touch.pointCount == 2)    // Two touch points
    {
        ...
        else if (event.touchAction == TOUCH_ACTION_UP)
        {
            ...
            GESTURES.Pinch.vector = (GETS(Vector2)){ 0.0f, 0.0f };
            GESTURES.Touch.pointCount = 0;
...
GETS(Vector2) GetGestureDragVector(void)
{
...
GETS(Vector2) GetGesturePinchVector(void)
{
...
static float rgVector2Angle(GETS(Vector2) v1, GETS(Vector2) v2)
{
...
// Calculate distance between two GETS(Vector2)
static float rgVector2Distance(GETS(Vector2) v1, GETS(Vector2) v2)
{
    float result;
