...
void DrawMesh(Mesh mesh, Material material, Matrix transform)
{
    if (mesh.animVertices) {
        rlEnableStatePointer(PF_VERTEX_ARRAY, mesh.animVertices);
        rlEnableStatePointer(PF_NORMAL_ARRAY, mesh.animNormals);
    } else {
        rlEnableStatePointer(PF_VERTEX_ARRAY, mesh.vertices);
        rlEnableStatePointer(PF_NORMAL_ARRAY, mesh.normals);
    }

    rlEnableStatePointer(PF_TEXTURE_COORD_ARRAY, mesh.texcoords);
    rlEnableStatePointer(PF_COLOR_ARRAY, mesh.colors);
    ...
    rlDisableStatePointer(PF_VERTEX_ARRAY);
    rlDisableStatePointer(PF_TEXTURE_COORD_ARRAY);
    rlDisableStatePointer(PF_NORMAL_ARRAY);
    rlDisableStatePointer(PF_COLOR_ARRAY);
...
Mesh GenMeshCylinder(float radius, float height, int slices)
{
    ...
    if (slices >= 3)
    {
        ...
        float f001[] = {0, 0, 1};
        float f000[] = {0, 0, 0};
        par_shapes_rotate(cylinder, -PI/2.0f, f100);

        // Generate an orientable disk shape (top cap)
        par_shapes_mesh *capTop = par_shapes_create_disk(radius, slices, f000, f001);
        capTop->tcoords = PAR_MALLOC(float, 2*capTop->npoints);
        for (int i = 0; i < 2*capTop->npoints; i++) capTop->tcoords[i] = 0.0f;
        par_shapes_rotate(capTop, -PI/2.0f, f100);
        par_shapes_rotate(capTop, 90*DEG2RAD, f010);
        par_shapes_translate(capTop, 0, height, 0);

        // Generate an orientable disk shape (bottom cap)
	float f00_1[] = {0, 0, -1};
        par_shapes_mesh *capBottom = par_shapes_create_disk(radius, slices, f000, f00_1);
        capBottom->tcoords = PAR_MALLOC(float, 2*capBottom->npoints);
        for (int i = 0; i < 2*capBottom->npoints; i++) capBottom->tcoords[i] = 0.95f;
        par_shapes_rotate(capBottom, PI/2.0f, f100);
        par_shapes_rotate(capBottom, -90*DEG2RAD, f010);
...
Mesh GenMeshCone(float radius, float height, int slices)
{
    ...
    if (slices >= 3)
    {
        ...
        float f000[] = {0, 0 ,0};
        float f00_1[] = {0, 0 ,-1};
        par_shapes_rotate(cone, -PI/2.0f, f100);
        par_shapes_rotate(cone, PI/2.0f, f010);
