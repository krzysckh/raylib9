...
PFcontext pfCreateContext(void* targetBuffer, PFsizei width, PFsizei height, PFpixelformat pixelFormat)
{
    ...
    PFframebuffer fb0 = {0,0};
    ctx->mainFramebuffer = fb0;
    ...
    ctx->clearColor = (PFcolor) { 0,0,0,0 };
    ...
    {
        memset(&ctx->lights[i].position, 0, sizeof(PFMvec3));
        memset(&ctx->lights[i].direction, 0, sizeof(PFMvec3));
        ctx->lights[i].innerCutOff = M_PI,
        ctx->lights[i].outerCutOff = M_PI,
        ctx->lights[i].attConstant = 1,
        ctx->lights[i].attLinear = 0,
        ctx->lights[i].attQuadratic = 0,
        ctx->lights[i].ambient = (PFcolor) { 51, 51, 51, 255 },
        ctx->lights[i].diffuse = (PFcolor) { 255, 255, 255, 255 },
        ctx->lights[i].specular = (PFcolor) { 255, 255, 255, 255 },
        ctx->lights[i].next = NULL;
    }
    ctx->activeLights = NULL;

    /* Initialization of fog properties */

    ctx->fog.mode = PF_LINEAR,
    ctx->fog.density = 1.0f,
    ctx->fog.start = 0.0f,
    ctx->fog.end = 1.0f,
    ctx->fog.color = (PFcolor) { 0,0,0,0 };

    /* Initialization of materials */

    ctx->faceMaterial[0] = ctx->faceMaterial[1] = (PFmaterial) {
        (PFcolor) { 255, 255, 255, 255 },
        (PFcolor) { 255, 255, 255, 255 },
        (PFcolor) { 255, 255, 255, 255 },
        (PFcolor) { 0, 0, 0, 255 },
    ...
    PFvertexattribs va0 = { 0,0,0,0 };
    ctx->vertexAttribs = va0;
...
void _pfClearColor(PFubyte r, PFubyte g, PFubyte b, PFubyte a)
{
    currentCtx->clearColor = (PFcolor) { r, g, b, a };
}

void pfClearColor(float r, float g, float b, float a)
{
  currentCtx->clearColor = (PFcolor) { (PFubyte)(r*255.f), (PFubyte)(g*255.f), (PFubyte)(b*255.f), (PFubyte)(a*255.f) };
}
...
// TODO: don't realloc-bump shit
static PFtexture **t_store = NULL;
static long t_store_n = 0;

unsigned int
pfStoreTexture(PFtexture *p)
{
  t_store = realloc(t_store, sizeof(PFtexture*) * (t_store_n+1));
  t_store[t_store_n] = p;

  // bodge that makes the indicies n+1, because raylib checks for id = 0 as a status of
  // texture not being loaded yet ffs
  return ++t_store_n;
}
